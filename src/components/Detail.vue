    ...
    <div class="Detail-top">
      ...
      <el-dialog title="添加公司" :visible.sync="dialogFormVisible">
        <el-form :model="form">
          ...
          <el-form-item label="公司图片" :label-width="formLabelWidth">
  ...
  methods: {
    ...
    submit(value) {
     ...
     this.dialogFormVisible=false;
      _this.http.get("amouse.index.contactsList", {
        display: "add",
        pcateid:23,
        info: value.info,
        title: value.title,
    ...
    delete1(id){
      console.log("哈哈",id);
      const _this=this;
      _this.$confirm("此操作将永久删除文件,是否继续?","提示",{
        confirmButtonText:"确定",
        cancelButtonText:"取消",
        type:"warning"
      }).then(()=>{
        _this.http.get("amouse.index.contactsList",{display:"slice",id:[id]}).then(res=>{
